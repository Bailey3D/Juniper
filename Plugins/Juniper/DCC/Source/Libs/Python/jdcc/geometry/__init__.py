...
import jdcc.scene
import jdcc.scene.selection_set_wrapper
...
class GeometryWrapper(juniper.types.wrappers.type_wrapper.TypeWrapper):
    ...
    def __validate_name(self, input_):
        ...
        if("{package}" in output):
            current_scene = jdcc.scene.get_current()
            if(current_scene):
    ...
    def __export(self):
        import pymxs.juniper.export as max_export
        ...
        if(isinstance(self.native_object, jdcc.scene.SceneWrapper)):
        ...
        elif(isinstance(self.native_object, jdcc.scene.selection_set_wrapper.SelectionSetWrapper)):
            success = max_export.export_nodes(
                [x.native_object for x in self.native_object.objects],
                export_output_path
            )
        ...
        elif(isinstance(self.native_object, jdcc.scene.object_wrapper.ObjectWrapper)):
