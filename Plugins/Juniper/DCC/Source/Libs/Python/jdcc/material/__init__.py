...
import jdcc.types.data.asset_interface
...
class MaterialWrapper(juniper.types.wrappers.type_wrapper.TypeWrapper):
    ...
    def load_asset_interface(self, asset_data_path):
        ...
        """
        if(asset_data_path):
            self.asset_interface = jdcc.types.data.asset_interface.AssetInterface(
                asset_data_path,
                create_if_invalid=True
            )
        else:
            self.asset_interface = jdcc.types.data.asset_interface.AssetInterface("", create_if_invalid=False)
    ...
    def _get_name(self):
        ...
        if("{package}" in output):
            ...
            package_name = sd.juniper.package.get_name(
    ...
    def _get_name(self):
        ...
        package_name = jdcc.scene.get_current().name
        output = output.replace("{package}", package_name)
        return output
    ...
    def _get_base_name(self):
        return jdcc.scene.get_current().name

    @get_base_name.override("designer")
    def _get_base_name(self):
        return sd.juniper.package.get_name(
            package=self.native_object.getPackage()
        )
    ...
    def _get_path(self):
        return jdcc.scene.get_current().path
    ...
    def __validate_exported_texture(self, texture_type, texture_path):
        ...
        new_fp = old_fp.replace("{package}", jdcc.scene.get_current().name)
        ...
        relative_exported_texture_path = string_utils.remove_prefix(
            ...
            os.path.join(unreal.juniper.unreal_project_dir(), "content")
        )
        relative_exported_texture_path = relative_exported_texture_path.lstrip("\\")
    ...
    def __export(self):
        ...
        if(export_output_dir and export_preset_name):
            ...
            import substance_painter.juniper.export_config
    ...
    def __export(self):
        ...
        if(self.asset_interface.get_metadata_key("#export:output_dir")):
            export_data = sd.juniper.graph.export_textures(
