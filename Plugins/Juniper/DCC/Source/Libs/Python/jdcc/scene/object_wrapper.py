...
class ObjectWrapper(juniper.types.wrappers.type_wrapper.TypeWrapper):
    ...
    def focus(self):
        ...
        """
        import jdcc.viewport
        jdcc.viewport.focus(self)
    ...
    def _copy(self):
        ...
        copy = pymxs.juniper.scene.clone(self.native_object, copy=True, deep_clone=True)
        if(len(copy)):
    ...
    def _instance(self):
        import pymxs.juniper.scene
        instance = pymxs.juniper.scene.clone(self.native_object, instance=True, deep_clone=True)
        if(len(instance)):
    ...
    def _reference(self):
        import pymxs.juniper.scene
        reference = pymxs.juniper.scene.clone(self.native_object, reference=True, deep_clone=True)
        if(len(reference)):
